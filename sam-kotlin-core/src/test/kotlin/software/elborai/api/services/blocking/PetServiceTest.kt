...
class PetServiceTest {
    ...
    fun callRetrieve() {
        val client = SamOkHttpClient.builder().baseUrl(TestServerExtension.BASE_URL).build()
        val petService = client.pets()
        val pet = petService.retrieve(PetRetrieveParams.builder().petId(123L).build())
        println(pet)
        pet.validate()
    }
    ...
    fun callDelete() {
        val client = SamOkHttpClient.builder().baseUrl(TestServerExtension.BASE_URL).build()
        val petService = client.pets()
        petService.delete(PetDeleteParams.builder().petId(123L).build())
    }
    ...
    fun callFindByTags() {
        val client = SamOkHttpClient.builder().baseUrl(TestServerExtension.BASE_URL).build()
        val petService = client.pets()
        val petFindByTagsResponse =
            petService.findByTags(PetFindByTagsParams.builder().tags(listOf("string")).build())
