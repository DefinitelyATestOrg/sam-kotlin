...
class ErrorHandlingTest {
    ...
    fun unexpectedStatusCode() {
      ...
      assertThatThrownBy({ client.stores().createOrder(params) }).satisfies({ e -> assertUnexpectedStatusCodeException(e, 999, ImmutableListMultimap.of("Foo", "Bar"), toJson(SAM_ERROR)) })
    }
    ...
    fun invalidBody() {
      ...
      assertThatThrownBy({ client.stores().createOrder(params) }).satisfies({ e -> assertThat(e)
    ...
    fun invalidErrorBody() {
      ...
      assertThatThrownBy({ client.stores().createOrder(params) }).satisfies({ e -> assertBadRequest(e, ImmutableListMultimap.of(), SamError.builder().build()) })
    }
